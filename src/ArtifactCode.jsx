...
<head>
    ...
    <style>
        ...
        h1, h2, h3, p, div, button, label, input, textarea {
...
<body class="min-h-screen flex items-center justify-center p-4">
                    ...
                    <div class="md:col-span-2 space-y-4">
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                <label for="character-interests" class="block text-lg font-medium text-gray-700">2. ما هي اهتمامات البطل؟</label>
                                <button id="develop-interests-btn" class="btn-ai-suggest">✨ طور الاهتمامات</button>
                            </div>
                        ...
                        <div>
                             <div class="flex justify-between items-center mb-2">
                                <label for="story-summary" class="block text-lg font-medium text-gray-700">3. اكتب ملخصاً للقصة</label>
                                <button id="suggest-idea-btn" class="btn-ai-suggest">✨ اقترح لي فكرة</button>
                            </div>
    ...
    <script>
        ...
        const developInterestsBtn = document.getElementById('develop-interests-btn');
        ...
        suggestIdeaBtn.addEventListener('click', suggestStoryIdea);
        developInterestsBtn.addEventListener('click', developInterests);
        ...
        /**
         * Calls Gemini to generate text based on a prompt.
        ...
        /**
         * ✨ Feature: Suggests a story idea using Gemini.
         */
        async function suggestStoryIdea() {
            const button = suggestIdeaBtn;
            button.disabled = true;
            button.textContent = 'جاري...';
            try {
                const prompt = "اقترح فكرة قصة قصيرة ومبتكرة لطفل في سطر واحد باللغة العربية.";
                const idea = await callGemini(prompt);
                document.getElementById('story-summary').value = idea.trim();
            } catch (error) {
                console.error("Error suggesting idea:", error);
                alert("عذراً، لم نتمكن من اقتراح فكرة. الرجاء المحاولة مرة أخرى.");
            } finally {
                button.disabled = false;
                button.textContent = '✨ اقترح لي فكرة';
            }
        }

        /**
         * ✨ Feature: Develops hero's interests using Gemini.
         */
        async function developInterests() {
            const interestsTextarea = document.getElementById('character-interests');
            const currentInterests = interestsTextarea.value.trim();
            if (!currentInterests) {
                alert("الرجاء كتابة بعض الاهتمامات أولاً.");
                return;
            }
            
            const button = developInterestsBtn;
            button.disabled = true;
            button.textContent = 'جاري...';
            try {
                const prompt = `طور هذه الاهتمامات لتصبح أكثر إبداعاً ومناسبة لقصة أطفال: "${currentInterests}". أعد كتابتها كقائمة وصفية وخيالية.`;
                const developedInterests = await callGemini(prompt);
                interestsTextarea.value = developedInterests.trim();
            } catch (error) {
                console.error("Error developing interests:", error);
                alert("عذراً، لم نتمكن من تطوير الاهتمامات. الرجاء المحاولة مرة أخرى.");
            } finally {
                button.disabled = false;
                button.textContent = '✨ طور الاهتمامات';
            }
        }
        ...
        async function downloadAsPDF() {
            ...
            for (let i = 0; i < bookData.length; i++) {
                ...
                switch (page.type) {
                    ...
                    case 'dedication':
                    case 'conclusion':
                         pageHtml = `<div style="width: ${pdfWidth}px; height: ${pdfHeight}px; display: flex; align-items: center; justify-content: center; padding: 40px; background: linear-gradient(180deg, #e0f2fe, #fff1f2);">
                                        <p style="font-family: 'Marhey', sans-serif; font-size: 24px; text-align: center; color: #374151; font-style: italic;">"${page.text}"</p>
                                    </div>`;
                        break;
                    case 'introduction':
                         pageHtml = `<div style="width: ${pdfWidth}px; height: ${pdfHeight}px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 24px; padding: 40px; background: linear-gradient(180deg, #d1fae5, #e0f2fe);">
